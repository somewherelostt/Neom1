...
export function WalletConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setHasManuallyDisconnected(false); // Reset disconnect flag on successful connect
      }
    } catch (error) {
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    setHasManuallyDisconnected(true); // Mark as manually disconnected
  }, [onConnectionChange]);
    ...
    if (window.ethereum) {
      const handleAccountsChanged = (accounts: string[]) => {
        if (accounts.length === 0) {
          ...
          // Account changed, reconnect only if not manually disconnected
          checkExistingConnection();
        }
      };
  ...
  }, [onConnectionChange, handleDisconnect, hasManuallyDisconnected]);
